 /*
 * Copyright 2015, 2020 Mats Eirik Hansen <mats.hansen@triumph.no>
 *
...
int RPIPCMessageWriteString(RPIPCMessageRef message, const char *stringValue)
{
    ...
    if(message)
    {
        ...
        if((messageHeader->contentLength + stringLength) < kRPIPCMaxContentLength)
        {
            strcpy((char *) &message->buffer[sizeof(RPIPCMessageHeader) + messageHeader->contentLength], stringValue);

            messageHeader->contentLength += stringLength + 1;
